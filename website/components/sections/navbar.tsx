...
export function Navbar() {
  ...
  useEffect(() => {
    const sections = [...new Set(links.map(([, href]) => href))]
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-accent/15 bg-background/95 backdrop-blur">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 lg:px-8" aria-label="Main navigation">
        <a href="#overview" onClick={(event) => onNavClick(event, "#overview")} className="flex items-center gap-2 text-lg font-bold tracking-tight">
        ...
        <div className="hidden items-center gap-6 lg:flex">
          {links.map(([label, href]) => (
            <motion.a
              ...
              className={`${linkClass} ${active === href ? "text-text-primary" : ""}`}
            >
              {label}
              {active === href && (
                <motion.span
            ...
            <div className="flex flex-col gap-1">
              {links.map(([label, href]) => (
                <motion.a
                  ...
                  key={label}
                  href={href}
                  onClick={(event) => onNavClick(event, href, true)}
                  className={`${linkClass} ${active === href ? "text-accent" : ""}`}
                >
                  {label}
                </motion.a>
              ))}
