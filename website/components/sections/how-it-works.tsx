...
export function HowItWorks() {
  ...
  return <section ref={sectionRef} id="how-it-works" className="how-scroll-section relative overflow-hidden py-24"><div ref={trackRef} className="relative z-10 how-scroll-track mx-auto flex max-w-7xl flex-col gap-8 px-5 lg:w-max lg:max-w-none lg:flex-row lg:gap-6 lg:px-0">{steps.map(([number, title, copy], index) => <motion.article key={number} className="how-step-panel rounded-lg border border-accent/15 bg-background-elevated p-8 lg:w-[72vw] lg:max-w-4xl" initial={reduceMotion ? false : { opacity: 0, y: 24 }} whileInView={reduceMotion ? {} : { opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }} transition={{ duration: .65, delay: index * .08, ease: "easeOut" }}><span className="font-mono text-xl text-accent">{number}</span><h3 className="mt-8 text-3xl font-bold">{title}</h3><p className="mt-4 max-w-xl text-lg leading-8 text-text-muted">{copy}</p></motion.article>)}</div></section>;
}
