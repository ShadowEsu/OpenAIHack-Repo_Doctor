...
function normalizeReview(value: Review): Review {
  ...
  return {
    ...
    findings: Array.isArray(value.findings) ? value.findings.slice(0, 8).map((finding, index) => ({
      id: `finding-${index + 1}`,
      title: String(finding.title || "Repository finding"),
      severity: severity.has(finding.severity) ? finding.severity : "medium",
...
export async function POST(request: Request) {
  try {
    ...
    const review = normalizeReview(await generateStructuredReview<Review>(
      "You are Repo Doctor, a conservative staff engineer. Analyze only the supplied public repository inventory and manifest excerpts. Never invent file contents, vulnerabilities, test coverage, line numbers, or credentials. Report at most 6 concrete, evidence-backed findings. Do not recommend destructive commands. Return strict JSON only.",
